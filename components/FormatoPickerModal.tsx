...
import type { Formato } from '@/lib/cart-types'

type FormatoPickerModalProps = {
  open: boolean
  selected: Formato
  onSelectChange: (value: Formato) => void
  onClose: () => void
  onConfirm: () => void
}

export default function FormatoPickerModal({
  open,
  selected,
  onSelectChange,
  onClose,
  onConfirm,
}: FormatoPickerModalProps) {
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="formato-modal-title"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <h2 id="formato-modal-title" className="font-title pr-10 text-2xl text-black">
          formato
        </h2>
        <p className="mt-2 font-body text-sm leading-relaxed text-black/80">
          Escolha o formato do marca-página Peixinho antes de adicionar ao carrinho.
        </p>

        <div className="mt-6 flex flex-col gap-3">
          {(['redondo', 'ondulado'] as Formato[]).map((opt) => (
            <button
              key={opt}
              type="button"
            ...
            >
              ...
              <span className="block text-xs text-black/60 mt-0.5">
                {opt === 'redondo'
                  ? 'cauda arredondada, linhas suaves'
                  : 'cauda ondulada, movimento natural'}
              </span>
            </button>
          ))}
        </div>
